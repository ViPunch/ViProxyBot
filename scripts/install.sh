...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: bash install.sh [--repo <url>] [--branch <name>]"
      exit 0
      ;;
...
[Unit]
Description=VPNBot Telegram service
...
EOF
  ...
  echo
  echo "VPNBot installed with systemd."
  echo "1. Edit:   nano ${ENV_FILE}"
  echo "2. Start:  systemctl restart vpnbot"
  echo "3. Logs:   journalctl -u vpnbot -f"
else
  echo
  echo "VPNBot installed."
  echo "1. Edit:   nano ${ENV_FILE}"
  echo "2. Start:  ${RUN_SCRIPT}"
  echo
  echo "systemd not detected (WSL?). Use run script above."
fi
